...
class Envelope:
    ...
    def __post_init__(self) -> None:
        ...
        if self.kind == "status":
            ...
            if set(normalized) != expected or normalized.get("source") not in {"radarr", "sonarr", "lidarr"} or not all(isinstance(normalized[name], str) and normalized[name] for name in expected - {"expected_bytes", "source"}) or not isinstance(selector, str) or len(selector) != 64 or any(char not in "0123456789abcdef" for char in selector) or isinstance(normalized["expected_bytes"], bool) or not isinstance(normalized["expected_bytes"], int) or normalized["expected_bytes"] <= 0:
