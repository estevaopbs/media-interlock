...
class FenceStore:
    ...
    def load(self, policy: FencePolicy) -> FenceState:
        ...
        allowed = {"reservations", "watermarks", "quiescing", "post_pnr_adoptions", "post_pnr_historical_adoptions", "post_pnr_historical_activations", "video_candidates", "music_candidates"}
        required = {"reservations", "watermarks", "quiescing"}
        if not isinstance(snapshot, dict) or not required <= set(snapshot) <= allowed or not isinstance(snapshot["reservations"], list) or not isinstance(snapshot["watermarks"], dict) or not isinstance(snapshot["quiescing"], bool) or ("post_pnr_adoptions" in snapshot and not isinstance(snapshot["post_pnr_adoptions"], list)) or ("post_pnr_historical_adoptions" in snapshot and not isinstance(snapshot["post_pnr_historical_adoptions"], list)) or ("post_pnr_historical_activations" in snapshot and not isinstance(snapshot["post_pnr_historical_activations"], list)) or ("video_candidates" in snapshot and not isinstance(snapshot["video_candidates"], list)) or ("music_candidates" in snapshot and not isinstance(snapshot["music_candidates"], list)):
            raise ContractError("durable Fence state is not a v2 snapshot")
        return FenceState.from_snapshot(policy, snapshot["reservations"], snapshot["watermarks"], quiescing=snapshot["quiescing"], post_pnr_adoptions=snapshot.get("post_pnr_adoptions", []), post_pnr_historical_adoptions=snapshot.get("post_pnr_historical_adoptions", []), post_pnr_historical_activations=snapshot.get("post_pnr_historical_activations", []), video_candidates=snapshot.get("video_candidates", []), music_candidates=snapshot.get("music_candidates", []))

    def save(self, state: FenceState) -> None:
        snapshot = {"reservations": state.records(), "watermarks": state.watermarks(), "quiescing": state.quiescing, "post_pnr_adoptions": state.post_pnr_records(), "post_pnr_historical_adoptions": state.post_pnr_historical_records(), "post_pnr_historical_activations": state.post_pnr_historical_activation_records(), "video_candidates": state.video_candidate_records(), "music_candidates": state.music_candidate_records()}
