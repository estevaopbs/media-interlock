    ...
    async def test_query_distinguishes_unavailable_conflict_and_catalog_confirmed_without_receipt(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            try:
                ...
                conflict = await self.exchange(path, publisher_assisted_complete(operation_id=operation_id, manifest=self.manifest()))
                self.assertEqual({"state": "conflict"}, dict(conflict.body))
            finally:
                server.close()
                await server.wait_closed()
