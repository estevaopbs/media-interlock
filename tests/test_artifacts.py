...
class ArtifactDefinitionTests(unittest.TestCase):
    ...
    def test_corrective_release_version_is_consistent(self) -> None:
        ...
        self.assertEqual("0.1.5", __version__)
    ...
    def test_packaged_readme_declares_the_current_immutable_release(self) -> None:
        ...
        self.assertIn("MediaInterlock 0.1.5 is the current immutable public", readme)
        self.assertIn("Release 0.1.4 remains preserved as its immutable predecessor.", readme)
        self.assertIn("MediaInterlock 0.1.5 is the immutable public downstream-consumption release.", state)
        self.assertIn("Version 0.1.4 remains preserved", state)
